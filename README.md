...
- Redesigned layout: a centered flex shell with a sticky, fixed-width left rail for DK-aware imports + the remaining board, and a right workspace that defaults to non-overlapping docked tabs (Board & Log / Win% Sims / Final Standings). The docked mode keeps everything readable on first load with no overlap against the left rail, even before toggling canvas mode.
- Optional canvas mode replaces the old freeform behavior: snap-to-grid drag/resize inside a padded canvas, clamped to the workspace so panels cannot spill left/right, with overlap resolution and per-tab saved layouts. Canvas is opt-in and migrations reset prior layouts to guarantee a clean default view.
